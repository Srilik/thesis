...
// import { StudentType } from "@/types/master";
import { StudentType } from "@/types/StudentType";

import { StudentType as BaStudentType } from "@/types/student";
import StudentMasterProfile from "./Partials/StudentMasterProfile.vue";
import StudentBaProfile from "./Partials/StudentBaProfile.vue";
import { onMounted } from "vue";
import { SearchIcon } from "lucide-vue-next";
import Student from "@/Pages/IdCard/Student.vue";
...
const isMasterComputed = computed(() => {
  const hasNumber = /\d/.test(keywords.value);
  const startsWithG =
    keywords.value?.startsWith("g") || keywords.value?.startsWith("G");
  return hasNumber && startsWithG;
});
...
interface Props {
  academicYear: string;
  semester: number;
  lecturers: ResourcePaginateType<LecturerType>;

  students?: StudentType[]; // Array of students
  keywords?: string;
  data?: string;
  masterStudent?: StudentType | null;
  baStudent?: BaStudentType | null;
  studentId?: string;
  filters?: {
    keyword?: string;
    thesis_id?: number;
...
watch(
  ...
  (value) => {
    if (value && !props.data) {
      if (isMasterComputed.value) {
        searchFrom.value = "master";
      } else {
        const studentIdNumber = parseInt(
          value.replace(/\D/g, "")?.substring(0, 4),
        );
        if (studentIdNumber >= 2010) {
          searchFrom.value = "new";
        } else if (studentIdNumber < 2010) {
          searchFrom.value = "old";
        } else {
          searchFrom.value = "new";
        }
      }
...
const onSaveThesis = () => {
  formThesis.post(route("thesis.management.thesises.store", formThesis.id), {
    onSuccess: async () => {
      Swal.fire({
        title: "Success",
        icon: "success",
...
const editThesis = async (thesisID: number) => {
  const { data } = await fetch<{}>(
    route("thesis.management.thesises.edit", thesisID),
  );
...
const onDeleteThesis = (thesisID: number) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      router.delete(route("thesis.management.thesises.delete", thesisID), {
        onSuccess: () => {
          Swal.fire({
            title: "Success",
            icon: "success",
...
const onSaveThesisCommittee = () => {
  formThesisCommittee.post(
    route("thesis.management.thesisCommittees.store", formThesisCommittee.id),
    {
      onSuccess: async () => {
        Swal.fire({
          title: "Success",
...
const editThesisCommittee = async (thesisCommitteeID: number) => {
  const { data } = await fetch<{}>(
    route("thesis.management.thesisCommittees.edit", thesisCommitteeID),
  );
...
const onDeleteThesisCommittee = (thesisCommitteeID: number) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      router.delete(
        route("thesis.management.thesisCommittees.delete", thesisCommitteeID),
        {
          onSuccess: () => {
            Swal.fire({
              title: "Success",
...
const onSaveThesisStudent = () => {
  formThesisStudent.post(
    route("thesis.management.thesisStudents.store", formThesisStudent.id),
    {
      onSuccess: async () => {
        Swal.fire({
          title: "Success",
...
const editThesisStudent = async (thesisStudentID: number) => {
  const { data } = await fetch<{}>(
    route("thesis.management.thesisStudents.edit", thesisStudentID),
  );
...
const onDeleteThesisStudent = (thesisStudentID: number) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      router.delete(
        route("thesis.management.thesisStudents.delete", thesisStudentID),
        {
          onSuccess: () => {
            Swal.fire({
              title: "Success",
...
const searchThesises = throttle(() => {
  router.get(route("thesis.management.index"), pickBy(filterForm.data()), {
    preserveState: true,
    only: ["thesises"],
    replace: true,
  });
...
<template>
    ...
    :breadcrumbs="[
      {
        ...
        url: route('thesis.index'),
      },
      {
        name: 'Examination  management',
        url: route('thesis.management.index'),
      },
    ]"
  ...
  >
    ...
    >
      ...
      <div class="w-full overflow-x-auto md">
        <div class="flex flex-col items-center gap-2 m-1 mb-6 md:flex-row">
          <Input
  ...
  >
    <div role="tablist" class="tabs tabs-lifted bg-gray-700">
      <input
        type="radio"
        name="my_tabs_2"
        role="tab"
        class="mr-2 font-bold tab"
        ...
        <div class="flex-1 p-2 overflow-auto bg-base-100">
          ...
          <div class="space-y-1">
            <div class="flex flex-col items-center gap-2 lg:flex-row">
              <div class="flex flex-col w-full">
                ...
                >
                  <option value="">Select a Student Year</option>
                  <option>1</option>
                  <option>2</option>
                  <option>3</option>
                  <option>4</option>
            ...
            <div class="flex flex-col items-center gap-2 lg:flex-row">
              ...
              <div class="flex flex-col w-full">
                <label class="label">
                  Topic in English<span class="text-error">*</span>
                </label>
                <textarea
            ...
            <div class="flex flex-col items-center gap-2 lg:flex-row">
              ...
              <div class="flex flex-col w-full">
                <label class="label"
                  >Objective in English<span class="text-error">*</span>
                </label>
                <textarea
          ...
          >
            ...
            <div class="flex flex-col items-center gap-2 lg:flex-row">
              ...
              <div class="flex flex-col w-full">
                <label class="label"
                  >Phone Number<span class="text-error">*</span></label
                >
                <input
            ...
            <div class="flex flex-col items-center gap-2 lg:flex-row">
              ...
              <div class="flex flex-col w-full">
                <label class="label"
                  >Address of Organization<span class="text-error"
                    >*</span
                  ></label
                >
                <input
          ...
          <div class="p-2 mt-1 bg-base-200 rounded-xl">
            <h4 class="text-lg text-center">Students Recently Added</h4>
            <div
            ...
            >
              <form @submit.prevent="onAddStudent">
                <input
                  ...
                  v-model="keywords"
                  type="text"
                  class="grow input input-primary mr-2"
                  placeholder="Search"
                  tabindex="0"
                  autofocus
                />
            ...
            <div class="mt-2 overflow-hidden border bg-base-100 rounded-xl">
              <div class="overflow-x-auto whitespace-nowrap">
                <table class="flex-col w-full overflow-x-auto base-table2 fle">
                  <thead>
                    <tr>
                      <th>Nº</th>
                      <th>Student ID</th>
                  ...
                  <tbody class="text-center">
                    <tr v-for="(item, index) in students" :key="item.id">
                      <td>{{ index + 1 }}</td>
                      <td class="font-bold text-blue-800">{{ item.id }}</td>
                      <td>{{ item.name }}</td>
                      <td>
                        <button
                          type="button"
      ...
      >
        ...
        <div class="p-2 mt-4 border-2 border-solid rounded-lg">
          ...
          <div class="flex flex-col items-center gap-2 lg:flex-row">
            <div class="flex flex-col w-full">
              <label class="label"
                >Advisor Name<span class="text-error">*</span></label
              >
              <input
                type="text"
                class="w-full input input-primary"
          ...
          <div class="flex flex-col items-center gap-2 lg:flex-row">
            <div class="flex flex-col w-full">
              <label class="label"
                >Dapartment<span class="text-error">*</span></label
              >
              <select class="w-full select select-primary">
          ...
          <div class="p-2 mt-2 bg-base-200 rounded-xl">
            <h4 class="text-lg text-center">Advisor Recently Added</h4>
            <div
              class="flex items-center justify-between gap-3 p-2 mt-2 bg-base-100 rounded-xl"
            >
              ...
              <div class="overflow-x-auto whitespace-nowrap">
                <table class="w-full base-table2">
                  ...
                  <tbody class="text-center">
                    <tr>
                      <td>1</td>
                      <td class="font-bold text-blue-800">Advisor Name</td>
                      <td>Male</td>
                      <td>
                        <button
                          type="button"
          ...
          <!-- Button Close & Save -->
          <div class="flex items-center justify-end gap-2 mt-2 border-t">
            <div class="space-x-1"></div>
            <div class="space-x-1">
              <Button
                class="bg-warning hover:bg-warning/90"
        ...
        <div class="p-2 mt-4 border-2 border-solid rounded-lg">
          ...
          <div class="flex flex-col items-center gap-2 lg:flex-row">
            <div class="flex flex-col w-full">
              <label class="label"
                >Committee Name<span class="text-error">*</span></label
              >
              <input
                type="text"
                class="w-full input input-primary"
          ...
          <div class="p-2 mt-2 bg-base-200 rounded-xl">
            <h4 class="text-lg text-center">Committee Recently Added</h4>
            <div
              class="flex items-center justify-between gap-3 p-2 mt-2 bg-base-100 rounded-xl"
            >
              <form @submit.prevent="onAddCommittee">
              ...
              <div class="overflow-x-auto whitespace-nowrap">
                <table class="w-full base-table2">
                  ...
                  <tbody class="text-center">
                    <tr>
                      <td>1</td>
                      <td class="font-bold text-blue-800">Committee Name</td>
                      <td>Male</td>
                      <td>
                        <button
                          type="button"
  ...
  >
    <div class="overflow-hidden shadow-xl bg-base-100 rounded-2xl">
      ...
      <div class="max-h-[calc(100vh-150px)] overflow-auto bg-base-100 p-2">
        <h2
        ...
        >
          <form class="w-full sm:w-auto" @submit.prevent="onAddAdvisor">
            <div class="flex flex-col gap-2 sm:flex-row sm:gap-3">
              <button class="w-full btn btn-success sm:w-auto">
                <CloudArrowDownIcon class="w-4 h-4" />Export
              </button>
              <button class="w-full text-white btn btn-error sm:w-auto">
                <PrinterIcon class="w-4 h-4" />Advisors Reported...
              </button>
            </div>
          </form>
          <div
            class="flex flex-col w-full gap-2 mt-2 overflow-x-auto sm:flex-row sm:w-auto sm:mt-0"
          >
            <!-- Input field -->
            <input
              type="text"
        ...
        <div class="p-2 mt-1 bg-base-200 rounded-xl">
          <h4 class="text-lg text-center">Advisor(s) Recently Added</h4>
          <div class="mt-2 overflow-hidden border bg-base-100 rounded-xl">
            <div class="overflow-x-auto whitespace-nowrap">
              <table class="flex-col w-full overflow-x-auto base-table2 flex">
                <thead>
                  <tr>
                    <th>Nº</th>
                    <th>Advisor Name</th>
                ...
                <tbody class="text-sm text-center">
                  <tr
                    ...
                    :key="lecturer.id"
                  >
                    <td>{{ index + lecturers.meta.from }}</td>
                    <td
                      class="font-bold text-blue-900 dark:bg-gray-800 dark:text-blue-300"
                    >
                      {{ lecturer.name }}
